"""Domain models for instruction and preference datasets.

Supports two dataset types:
  - Instruction: (instruction, answer) pairs for SFT training
  - Preference: (instruction, rejected, chosen) triples for DPO training
"""
...
try:
    ...
    logger.warning("Huggingface datasets not installed or lzma missing. HF export will be unavailable.")
    Dataset = None
    DatasetDict = None
...
class InstructDataset(VectorBaseDocument):
    ...
    def to_huggingface(self) -> "Dataset":
        assert Dataset is not None, "datasets library not available — install it or fix lzma"
...
class TrainTestSplit(VectorBaseDocument):
    ...
    def to_huggingface(self, flatten: bool = False) -> "DatasetDict":
        assert DatasetDict is not None, "datasets library not available — install it or fix lzma"
