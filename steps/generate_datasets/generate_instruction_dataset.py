"""Step: Generate instruction dataset (SFT) from prompts via LLM."""
...
def generate_instruction_dataset(
    ...
    step_context.add_output_metadata(output_name="instruct_datasets", metadata=_get_metadata(datasets))

    return datasets


def _get_metadata(datasets: InstructTrainTestSplit) -> dict[str, Any]:
